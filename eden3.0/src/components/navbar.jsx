...
function NavBar() {
	...
	return (
		<nav className="flex flex-row justify-between h-[90px] w-full z-50 text-white shadow-sm fixed top-0 left-0 md:px-20 ">
			...
			{isOpen && (
				<div className="md:hidden">
					<div onClick={openDrawer} className="w-full h-screen top-0 left-0 md:hidden bg-black absolute bg-opacity-20"></div>
					<div className="flex flex-col border border-[#FF9F2F] rounded-xl  w-full shadow-lg h-fit min-h-[50%] fixed top-0 left-0 gap-0 py-10 justify-center items-center">
						...
						{menuItems.map((value, index) => (
							<Link to={value.url} onClickCapture={openDrawer} className="text-md text-center  border-b border-gray md:border-0 p-4 py-6 md:p-0  flex items-center justify-center h-full cursor-pointer w-full hover:bg-gray md:hover:bg-[#ffffff00] md:hover:text-primary" key={index} onClick={() => navigate(value.url)}>
								{value.text}
							</Link>
						))}
					</div>
